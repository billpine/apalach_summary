...
disE  = dis[dis$dates>='1950-01-01' & dis$dates<='2018-12-31',]  
...
disE.mo.ts  = ts(disE.mo$avg,start=c(1950,1),end=c(2018,12),frequency=12)
disE.mo.sum.ts  = ts(disE.mo$sumflow,start=c(1950,1),end=c(2018,12),frequency=12)
disE.yr.ts  = ts(disE.yr$avg,start=1950,end=2018,frequency=1)
...
#discharge data, 1950-2018
hydroplot(disE.zoo,FUN=mean,ylab='Q',var.unit='cf/s')
title(paste(stinfo[1,2],': 1950-2018',sep=''),outer=T)
...
dis.fdc = as.data.frame(cbind(ts(disE$val[disE$year<2019]),ts(disE$val[disE$year>=2004&disE$year<=2014])))
fdc(dis.fdc,ylab='Flow (cf/s)',leg.txt=c('1950-2018','2005-2013'),cex.axis=1)
title(stinfo[1,2],outer=T)

dis.fdc = as.data.frame(cbind(ts(disE$val[disE$year<2019]),ts(disE$val[disE$year>=2013])))
fdc(dis.fdc,ylab='Flow (cf/s)',leg.txt=c('1950-2018','2014-2018'),cex.axis=1)
title(stinfo[1,2],outer=T)
...
plot(disE.yr$year,disE.yr$avg,type='p',xlab='Date',ylab='Discharge (cf/s)',pch=19,cex=1.5,main='Mean daily discharge by year', ylim=c(0,45000))
...
plot(disE.yr$year,disE.yr$var,type='p',xlab='Date',ylab='Discharge (cf/s)',pch=19,cex=1.5,main='Daily discharge variance by year', ylim=c(0,max(disE.yr$var)))
lines(lowess(disE.yr$year,disE.yr$var),col='red', lwd=4)
abline(h=disE.all[4], col="blue", lwd=4, lty=2)
#abline(lm(disE.yr$avg~seq(1950,2018,1)))
title(outer=T)

plot(disE.yr$year,disE.yr$CV,type='p',xlab='Date',ylab='Discharge (cf/s)',pch=19,cex=1.5,main='Daily discharge CV by year', ylim=c(0,120))
