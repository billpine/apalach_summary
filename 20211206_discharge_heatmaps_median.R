...
#get monthly sum, median, sd, and var
#discharge
disE.mo  = aggregate(val~month+year,data=disE,FUN = function(x) c(median(x,na.rm=T),sd(x,na.rm=T),var(x,na.rm=T),sum(x)))
disE.mo  = do.call('data.frame',disE.mo)
names(disE.mo)[3:6] = c('median','sd','var','sumflow') 
disE.mo$yrmo = disE.mo$year+(disE.mo$month-0.5)/12       

#value just by month
disE.month  = aggregate(val~month,data=disE,FUN = function(x) c(median(x,na.rm=T),sd(x,na.rm=T),var(x,na.rm=T),sum(x)))
disE.month  = do.call('data.frame',disE.month)
names(disE.month)[2:5] = c('median','sd','var','sumflow')

#calculate how far each month average is from the overall average - absolute difference
for(i in 1:nrow(disE.mo)){
        m = disE.mo$month[i]
        disE.mo$diff[i] = disE.mo$median[i] - disE.month$median[disE.month$month == m]
        disE.mo$diff_percent[i] = ((disE.mo$median[i] - disE.month$median[disE.month$month == m])/disE.month$median[disE.month$month == m])*100
}
...
plot(dis_per2005, xlab = "Month", ylab = "Year",
     breaks = c(-100, -50, -25, -10, 10, 25, 50, 100, 150, 200, 250, 300, 350, 400, 450),
     main = "Deviations in Median Discharge from Period of Record",
     axis.row = list(las = 2),
     col = colAll, na.col = "black")
