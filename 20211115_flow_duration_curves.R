...
disE  = dis[dis$dates>='1920-10-01' & dis$dates<='2021-11-31',]  
...
dis.fdc = as.data.frame(cbind(ts(disE$val[disE$year<2022]),ts(disE$val[disE$year>2004&disE$year<2013]),
                              ts(disE$val[disE$year > 2012])))


#create myself (not using fdc() function)
all <- sort(dis.fdc$`ts(disE$val[disE$year < 2022])`, decreasing = T)
part <- sort(dis.fdc$`ts(disE$val[disE$year > 2004 & disE$year < 2013])`,decreasing = T)
part2 <- sort(dis.fdc$`ts(disE$val[disE$year > 2012])`, decreasing = T)

df_all <- data.frame(x = 100/length(all) * 1:length(all), y = all)
df_part_2005_2012 <- data.frame(x = 100/length(part) * 1:length(part), y = part)
df_part_2013_2022 <- data.frame(x = 100/length(part2) * 1:length(part2), y = part2)
...
lines(x = df_part_2005_2012$x, y = log(df_part_2005_2012$y), type = 'l', col = 'red', lwd = 2)
abline(h = log(5000), lwd = 2, col = 'blue', lty = 2)
legend("topright", legend = c("1930-2020", "2005-2012"), lty = 1, col = c("black", "red"))
axis(1, labels = c("0%", "20%", "40%", "60%", "80%", "100%"), at = c(0, 20, 40, 60,80, 100))

plot(x = df_all$x, y = log(df_all$y), type = 'l', lwd = 2, ylab = "Log(Flow (cf/s))", 
     xlab = "% Time flow equalled or exceeded", xaxt = 'n',main='Discharge duration')
lines(x = df_part_2013_2022$x, y = log(df_part_2013_2022$y), type = 'l', col = 'red', lwd = 2)
legend("topright", legend = c("1930-2022", "2013-2022"), lty = 1, col = c("black", "red"))
abline(h = log(5000), lwd = 2, col = 'blue', lty = 2)
axis(1, labels = c("0%", "20%", "40%", "60%", "80%", "100%"), at = c(0, 20, 40, 60, 80, 100))
...
df_part_2005_2012_2 <- df_part_2005_2012
df_part_2005_2012_2$x <- round(df_part_2005_2012_2$x, digits = 2)
df_part_2005_2012_3 <- aggregate(df_part_2005_2012_2, by = list(df_part_2005_2012_2$x), FUN = 'mean')

df_merge  <- merge(df_part_2005_2012_3, df_all3, by.x = "x", by.y = "x")

length(which(log(df_merge$y.x) < log(df_merge$y.y)))
length(df_merge$y.x)

#0.965 (2820/2922) #this depends on which gauge and years used
...
########
## how often is the current values more than 10% below the average?


#how often 2005-2012 is below?
df_merge$y.y10 <- df_merge$y.y * 0.90
length(which(log(df_merge$y.x) < log(df_merge$y.y10)))#2687
length(df_merge$y.x)#2922
#2687/2922 = 92%

#how often 2013-2021 below?
df_merge2$y.y10 <- df_merge2$y.y * 0.90
length(which(log(df_merge2$y.x) < log(df_merge2$y.y10))) #0
length(df_merge2$y.x)#2922
#0/2922 = 0%
